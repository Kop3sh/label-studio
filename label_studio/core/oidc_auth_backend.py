...
from label_studio.users.functions import save_user_info
...
class OIDCAuthBackend(OIDCAuthenticationBackend):
    ...
    def create_user(self, claims):
        ...
        logger.debug("Creating %s user with claims: %s", username, claims)
        ...
        # user = super(PermissionBackend, self).create_user(claims)

        user_model = get_user_model()
        user = user_model.objects.create_user(email)
        user.first_name = claims.get('given_name', '')
        user.last_name = claims.get('family_name', '')
        user = save_user_info(user)
    ...
    def update_user(self, user, claims):
        ...
        logger.debug("Creating %s user with claims: %s", user, claims)
